...
(function () {
	...
	for (var i = 0; i < lines.length; i++) {
		...
		name2bounds[data[0]] = [parseInt(data[1].trim()), parseInt(data[2].trim()),
	...
	module.exports.run = function (args, noDataValue, tileDir, tilePrefix, outfile, buffer = 0,
		rescale = false, rescaleRange = 1, rescaleReverse = false) {
		// get the input data
		var inputData = '';
		try {
		...
		if (inputData.output_format != 'tif') {
			...
			const infoObj = JSON.parse(output);

			var scale = 1;
			var offset = 0;
			if (rescale) {
				/*
				y = mx + c, range 0 to R-1 (rescale range R)
				x=x_min => y=0
				x=x_max => y=R-1
				So m = dy/dx = (R-1)/(x_max-m_min)
				At x_min, 0 = m.x_min + c, c = -m.x_min
				*/
				scale = (rescaleRange - 1) / (infoObj.bands[0].maximum - infoObj.bands[0].minimum);
				offset = -scale * infoObj.bands[0].minimum;
				if (rescaleReverse) {
					/*
					x=x_max => y=0
					x=x_min => y=R-1
					So m = dy/dx = (R-1)/(x_min-x_max), i.e. negative above
					At x_max, 0 = m.x_max + c, c = -m.x_max
					*/
					scale *= -1;
					offset = -scale * infoObj.bands[0].maximum;
				}
			}
			...
			var summaryStdDev = infoObj.bands[0].stdDev;
			if (summaryMin !== undefined) { summaryString += `"min": ${summaryMin},`; }
			if (summaryMax !== undefined) { summaryString += `"max": ${summaryMax},`; }
			if (summaryMean !== undefined) { summaryString += `"mean": ${summaryMean},`; }
			if (summaryStdDev !== undefined) { summaryString += `"stdDev": ${summaryStdDev},`; }
			...
			// data is stored in javascript at any point.
				...
				lr.on('line', (line) => {
					...
					if (lineSplit[0] == 'ncols') {
						...
						for (var i = 0; i < lineData.length; i++) {
							lineData[i] = Math.round(scale * lineData[i] + offset);
						}
